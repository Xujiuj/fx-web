...
export function AdminMenuPageManager({ items, linkedPageCounts, embedded = false }: { items: NavItem[]; linkedPageCounts: number[]; embedded?: boolean }) {
  ...
  const table = <ProTable<MenuPageRow> rowKey="id" headerTitle="一级菜单栏目" dataSource={rows} columns={columns} search={false} scroll={{ x: "max-content" }} pagination={{ pageSize: 10, showSizeChanger: true }} options={{ density: true, fullScreen: true, reload: false, setting: true }} />;
