...
export function AdminMenuPageManager({ items, linkedPageCounts }: { items: NavItem[]; linkedPageCounts: number[] }) {
  ...
  return <PageContainer title="页面管理" content="按一级菜单分别维护其直属链接和二级菜单对应的页面内容。"><ProTable<MenuPageRow> rowKey="id" headerTitle="一级菜单栏目" dataSource={rows} columns={columns} search={false} pagination={{ pageSize: 10, showSizeChanger: true }} options={{ density: true, fullScreen: true, reload: false, setting: true }} /></PageContainer>;
}
