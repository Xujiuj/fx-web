...
export function HonorsPage({ page }: AboutPageProps) {
  ...
  return (
    <>
      ...
      <section className={styles.content} aria-label={section.title}>
        {section.items.length ? (
          <div className={styles.honorsGrid}>
            {section.items.map((item) => (
              <article className={`${styles.honorCard} page-reveal`} key={`${item.title}-${item.image ?? ""}`}>
                <h2>{item.title}</h2>
      ...
      <section className={styles.content} aria-label={section.title}>
        {section.items.length ? (
          <div className={styles.partnerGrid}>
            {section.items.map((item) => (
              <article className={`${styles.partnerCard} page-reveal`} key={`${item.title}-${item.image ?? ""}`}>
      ...
      <section className={styles.content} aria-label={section.title}>
        <div className={styles.contactGrid}>
          {section.items.map((item) => {
            ...
            return (
              <article className={`${styles.contactCard} page-reveal`} key={`${item.title}-${item.value ?? ""}`}>
                <h3>{item.title}</h3>
