...
function normalizeKnowledgeEntries(entries: unknown): KnowledgeEntry[] {
  ...
  return valid.length ? valid.map((entry) => {
    const fallback = defaultKnowledgeEntries.find((item) => item.slug === entry.slug);
