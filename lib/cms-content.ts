...
// bindings migrate only the known legacy diagram paths; a later administrator
// upload remains untouched and continues to drive the page.
...
const legacySolutionDiagramPaths: Record<string, ReadonlySet<string>> = {
  ...
  "solution-practical": new Set([
    ...
    "/media/product-excel-hero.webp",
    "/media/solution-practical-generated.png",
    "/media/solution-platform-generated.png",
  ]),
...
const legacySolutionDiagramContents: Record<string, ReadonlySet<string>> = {
  ...
  "solution-practical": new Set([
    "企业温室气体核算敏捷实施技术路线",
    "将温室气体核算方法、数据模型与分析应用逐步落地，构建可配置、可复用、可持续演进的数字化核算能力。",
    "从项目准备、数据建模到成果交付，明确首次核算闭环各阶段的工作事项与交付结果。",
  ]),
  "solution-platform": new Set([
    "企业碳管理平台架构图",
    "企业碳管理平台功能架构",
    "企业碳管理平台的技术路线与功能协同关系。",
    "构建从核算基础数据管理、排放分析到经营决策应用的完整能力体系，实现碳数据向管理价值的持续转化。",
  ]),
};
...
function normalizeStoredSubpages(subpages: Subpage[]): Subpage[] {
  return subpages.map((page) => {
    ...
    return media !== page.media || sections.some((section, index) => section !== page.sections[index])
      ? { ...page, media, sections }
      : page;
  });
}
...
async function loadSubpages(): Promise<Subpage[]> {
  ...
  try {
    ...
    const content = record ? parseSiteContentDocument<unknown>(record.value) : null;
    return isCurrentSubpages(content, defaultSubpages) ? normalizeStoredSubpages(content) : defaultSubpages;
  } catch {
...
export async function getSiteContentBundle(): Promise<SiteContentBundle> {
  ...
  try {
    ...
    return {
      ...
      subpages: isCurrentSubpages(storedSubpages, defaultSubpages) ? normalizeStoredSubpages(storedSubpages) : defaultSubpages,
